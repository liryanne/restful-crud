(ns restful-crud.restful
  (:require [compojure.api.sweet :refer [POST GET PUT DELETE routes]]
            [toucan.db :as db]
...
(defn create-route
  [{:keys [name model req-schema enter]}]
...
(defn get-by-id-route
  [{:keys [name model leave]}]
